...
export const lanTransferService = {
  ...
  approvePairing(deviceId: string, code: string) { requireDesktop(); return invokeCommand<{state:string;device_id:string|null;device_name:string|null;message:string}>("approve_lan_pairing", { deviceId, code }, null as unknown as {state:string;device_id:string|null;device_name:string|null;message:string}); },
  pairedDevices() { requireDesktop(); return invokeCommand<LanDevice[]>("get_paired_lan_devices", {}, []); },
  sendBackup(backupId: number, deviceId: string) { requireDesktop(); return invokeCommand<void>("send_lan_backup", { backupId, deviceId }, undefined); },
  testConnection(deviceId: string) { requireDesktop(); return invokeCommand<LanDevice>("test_lan_connection", { deviceId }, null as unknown as LanDevice); },
  revokeDevice(deviceId: string) { requireDesktop(); return invokeCommand<void>("revoke_lan_device", { deviceId }, undefined); },
  importBackup(token: string, backupId: number) { requireDesktop(); return invokeCommand<{success:boolean;message:string}>("import_lan_backup", { token, backupId }, null as unknown as {success:boolean;message:string}); },
