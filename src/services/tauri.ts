...
function previewInvoke<T>(command: string, args: Record<string, unknown> | undefined, fallback: T): T {
  ...
  if (command === "delete_payroll_period") { set("payrollPeriods", list<Record<string,unknown>>("payrollPeriods").filter(x=>x.id!==input.periodId)); return {success:true,message:"Payroll period deleted in preview",id:input.periodId} as T; }
  if (command === "delete_payroll_history") { const pid=Number(input.periodId); set("payrollPeriods", list<Record<string,unknown>>("payrollPeriods").filter(x=>Number(x.id)!==pid)); set("payrollRecords", list<Record<string,unknown>>("payrollRecords").filter(x=>Number(x.period_id)!==pid)); set("payslips", list<Record<string,unknown>>("payslips").filter(x=>Number(x.period_id)!==pid)); return {success:true,message:"Payroll history deleted in preview",id:pid} as T; }
  if (command === "clear_payroll_history") { const historyIds=new Set(list<Record<string,unknown>>("payrollPeriods").filter(x=>x.status==="closed"||x.status==="locked").map(x=>Number(x.id))); set("payrollPeriods", list<Record<string,unknown>>("payrollPeriods").filter(x=>!historyIds.has(Number(x.id)))); set("payrollRecords", list<Record<string,unknown>>("payrollRecords").filter(x=>!historyIds.has(Number(x.period_id)))); set("payslips", list<Record<string,unknown>>("payslips").filter(x=>!historyIds.has(Number(x.period_id)))); return {success:true,message:"Payroll history cleared in preview",id:null} as T; }
  ...
  if (command === "delete_loan") { const lid=Number(input.loanId); set("loans", list<Record<string,unknown>>("loans").filter(x=>Number(x.id)!==lid)); return {success:true,message:"Loan / advance deleted in preview",id:lid} as T; }
  if (command === "clear_loans") { set("loans", []); return {success:true,message:"All loans / advances deleted in preview",id:null} as T; }
